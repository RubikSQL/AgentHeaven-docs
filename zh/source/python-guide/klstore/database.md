...
DatabaseKLStore 是一个持久化的 [BaseKLStore](./base.md) 实现,通过 SQLAlchemy ORM 由 SQL 数据库支持。它为知识对象提供符合 ACID 的、可扩展的存储,支持 PostgreSQL、MySQL、SQLite、DuckDB 和 MSSQL 等。
...
- **`provider`**(可选):数据库提供商("sqlite"、"pg"、"mysql"、"duckdb"、"mssql");如果省略则使用配置默认值
